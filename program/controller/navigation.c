...
void PID_Nav(nav_pid_t *PID_nav,attitude_t *attitude,UBXvelned_t *UBXvelned, UBXposLLH_t *UBXposLLH){
	...
	(PID_nav -> output_roll) = P_roll_control+D_roll_control;
	(PID_nav -> output_pitch) = P_pitch_control+D_pitch_control;

	(PID_nav -> output_roll) = bound_float(PID_nav -> output_roll,-50.0f,50.0f);
	(PID_nav -> output_pitch) = bound_float(PID_nav -> output_pitch,-50.0f,50.0f);
